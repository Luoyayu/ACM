...
inline bool check(int x) {
    if (vis[x] == 0 && x < maxn && x >= 0) return 1;
    return 0;
}
void bfs(int s) {
    ...
    while (!q.empty())  // if open_table is not empty,else meaning all node
                        // hased been visted
    {
        int now = q.front();
        q.pop();  // take current node in queue front
        for (int i = 0; i < 3;
             i++)  // try all tests ,make all connected node into open_table
        {
            if (i == 0)
                nx = now + 1;
            else if (i == 1)
