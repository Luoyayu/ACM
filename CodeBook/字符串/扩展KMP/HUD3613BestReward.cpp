...
void get_next(char *a, int len) {
    ...
    next[1] =
        k;  //这里预先算好next[1]是因为不能k=0,否则next[i-k]=next[i]不是已算好的
    k = 1;
...
void EKMP(char *a, char *b, int *extend, int len) {
    ...
    while (++i < len) {
        ...
        extend[i] = min(next[i - k],
                        max(maxr - i + 1,
                            0));  // next[i-k]是a与b从i开始的可能已经匹配的长度
        while (i + extend[i] < len && a[extend[i]] == b[i + extend[i]])
            ++extend[i];  //这里是扩展KMP的精髓,即算法核心思想就是这
...
int main() {
    ...
    while (t--) {
...
#ifdef Local
        for (int i = 0; i < strlen(s1); i++) printf("%d ", ex1[i]);
        printf("\n");

        for (int i = 0; i < strlen(s2); i++) printf("%d ", ex2[i]);
        printf("\n");
#endif  // Local
        ...
        for (int i = 1; i < len; ++i) {
            ...
            if (ex2[i] == len - i)
                temp += sum[len] - sum[i];  //表示后len-i个字符为回文串
