...
bool smallerThan(int i,int j)//判断将s2插入到s1的第i位是否插入到第j位小
{
    if(ex[j]<i-j)
        return s1[j+ex[j]]<s2[ex[j]];
    if(_next[i-j]<ls2-(i-j))
        return s2[_next[i-j]]<s2[i-j+_next[i-j]];
    if(_next[ls2-(i-j)]<(i-j))
        return s2[ls2-(i-j)+_next[ls2-(i-j)]]<s2[_next[ls2-(i-j)]];
    return false;
}
int main()
{
    while(scanf("%s%s",s1,s2)!=EOF)
    {
        ...
        for(int i=0;i<ls1;i++)
        {
            if(smallerThan(i+1,ans))//如果插入到第i+1位比插入到第ans位小的话
                ans=i+1;//新的可行位置
        }
        //占位[ans,ans+ls1]
        for(int i=0;i<ans;i++)
