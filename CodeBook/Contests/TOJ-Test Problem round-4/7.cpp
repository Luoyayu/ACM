...
{
    int i=0;ll miss=1;//miss为[o,n]中缺少的最小整数
    int cnt=0;
    while(miss<=n)
    {
...
1. 如果数组中有某个整数x<=miss, 那么我们可以把[0,miss)区间的所有整数加上x，区间变成了[x, miss x)，由于区间[0,miss)和[x, miss x)重叠，两个区间可以无缝连接起来，意味着我们可以把区间[0,miss)扩展到[0, miss x)。
2. 如果数组中不存在小于或等于miss的元素，则区间[0,miss)和[x, miss x) 脱节了，连不起来。此时我们需要添加一个数，最大限度的扩展区间[0, miss)。那添加哪个数呢？当然是添加miss本身，这样区间[0,miss)和[miss, miss miss)恰好可以无缝拼接。
