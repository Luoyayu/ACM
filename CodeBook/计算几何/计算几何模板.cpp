...
struct Point {
    ...
    Point operator*(double b) const { return Point(b * x, y * b); }
    Point operator/(double b) const { return Point(x / b, y / b); }
    double operator^(const Point b) const {  //叉积
        return x * b.y - y * b.x;
    }
    void input() { scanf("%lf %lf", &x, &y); }
    void output() { printf("Point/Vector= (%lf,%lf)\n", x, y); }
...
bool point_on_segment(Point P, Line L) {
    return sgn(cross((L.s - P), (L.e - P))) == 0 &&
...
bool is_line_inter(Line l1, Line l2) {
    return max(l1.s.x, l1.e.x) >= min(l2.s.x, l2.e.x) &&
           ...
           sgn((cross((l2.s - l1.e), (l1.s - l1.e)))) *
                   sgn(cross((l2.e - l1.e), (l1.s - l1.e))) <=
               0 &&
           sgn(cross((l1.s - l2.e), (l2.s - l2.e))) *
                   sgn(cross((l1.e - l2.e), (l2.s - l2.e))) <=
               0;
}
