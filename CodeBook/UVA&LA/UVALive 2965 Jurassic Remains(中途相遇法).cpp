...
//题意：给出字符串组s1~sn(n<=24), 问如何尽可能的选字符串使得每个字符出现的次数均为偶数
...
int A[maxn];
map<int, int> table;
int bitCount(int x) { return !x ? 0 : bitCount(x>>1) + (x&1); } //返回集合大小
int main()
{
    int n;
    while(scanf("%d", &n) != EOF)
    {
        for(int i = 0; i < n; i++)
        {
            scanf("%s", s); 
            A[i] = 0;
            for(int j = 0; s[j]; j++) A[i] ^= (1 << (s[j] - 'A')); //xor count奇偶
        }
        ...
        for(int i = 0; i < n2; i++)
        {
            ...
            {
                if(i & (1<<j)) x ^= A[n1+j];
                if(table.count(x) && bitCount(x) <  bitCount(table[x])+bitCount(i)) 
                    ans=(i<<n1) ^ table[x];
            }
        }
        printf("%d\n", bitCount(ans));
