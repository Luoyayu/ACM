...
bool cmp(staff a, staff b) { return a.ability > b.ability; }
...
void DFS(int cur)  // DFS遍历，记录其子树的起始编号和终止编号
{
    ...
    for (int i = head[cur]; i != -1; i = D[i].next) DFS(D[i].to);
...
void update(int id, int val, int node, int l, int r)  //节点加入线段树
{
    ...
    int m = (l + r) >> 1;
    if (id <= m)
        update(id, val, lson, l, m);
    else
        update(id, val, rson, m + 1, r);
        ...
        for (int i = 1; i < N; i = j) {
            ...
            while (j < N && People[i].ability == People[j].ability) {
                ...
                if (sub[id].st + 1 <= sub[id].ed - 1)
                    tp = query(1, 1, dfs_clock - 1, sub[id].st + 1,
                               sub[id].ed - 1);
                if (tp == -1)
                    ans[id] = -1;
                else
