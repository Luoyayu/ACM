 ...
 * 朴素做法O(n*m) ans[0,1,2]维护00 01 11这样的边
 * 单点修改一次修改与之相邻的点的ans 莫队改进复杂度(0.5*n*sqrt(m))
 ...
 */
int deg[maxn], sp[maxn], type[maxn], g[maxn][2], v[maxn << 2], nxt[maxn], ed,
    sqr;
    ...
    while (scanf("%d%d", &n, &m) != EOF) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (edge[i].u > edge[i].v)
                swap(edge[i].u, edge[i].v);  //确保从小指向大点
        }
        sort(edge, edge + m);
        ...
        for (int i = 0, j; i < m; i = j) {
            for (j = i + 1;
                 j < m && edge[i].u == edge[j].u && edge[i].v == edge[j].v; j++)
                edge[i].w += edge[j].w;
            edge[cnt++] = edge[i];
        }
        ...
        for (int i = 0; i < cnt; i++) deg[edge[i].u]++, deg[edge[i].v]++;

        for (int i = 1; i <= n; i++) sp[i] = (deg[i] >= sqr);
