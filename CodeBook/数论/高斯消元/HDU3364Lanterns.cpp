#include <bits/stdc++.h>
//题意给出N个灯笼
//m个开关,q次对于末状态的询问，问有几种开关灯的方式，每盏灯可能被多个开关控制，同时每个开关可能控制多盏灯
//思路，n个灯笼就对应着n个方程，m个开关对应着m个未知数，方程得系数就是对于第i盏灯，能否被第j个开关控制，能的话系数为1否则为0
//令做一个数组存下初始状态，每次询问时补上最后一列，求出得未知变元得个数ans
//即为不清状况的数量那么答案为 2*ans
using namespace std;
const int maxn = 55;
...
int Gauss() {
    ...
    for (k = 0; k < equ && col < var; ++k, ++col) {
        ...
        if (MaxRow != k) {
            for (int j = k; j < var + 1; ++j) swap(a[k][j], a[MaxRow][j]);
        }
        if (a[k][col] == 0) {
            k--;
            continue;
