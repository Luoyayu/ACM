...
bool notprime[maxn];//notprime[i]==0表示i是质数,默认全是质数
int prime[maxn], tot=0;//p[maxn]用来存质数 tot从0计数
void getPrime()
{
    for(int i = 2; i <= maxn; i++)
    {
        if(!notprime[i]) prime[tot++] = i;//把质数存起来
        for(int j = 0; j < tot && i * prime[j] < maxn; j++)
        {
            notprime[i * prime[j]] = true;
            if(i % prime[j] == 0) break;//确保每个合数只被它最小的质因数筛去
        }
    }
    return ;
}
