...
int CRT() {
    ...
    for (int i = 1; i < m; i++) {
        ...
        int m2 = a[i], r2 = b[i];
        int d =
            ex_gcd(m1, m2, x, y);  //求出gcd(m1.m2) 解的m1*x + m2*y=gcd(m1,m2)
        int c = r2 - r1;
        if (c % d) {
            flag = 1;
            break;
...
bool modular_liner_equation(int a, int b,
                            int n)  // ax==b(mod n) 的充要条件是n|ax-b
{
    int x, y;  //设ax-b=yn ax-ny=b  r=gcd(a,n)有r个解
    int gcd = ex_gcd(a, n, x, y);
    if (b % gcd) return false;
    int x0 = x * (b / gcd) % n;
    for (int i = 1; i <= gcd; i++) printf("%d\n", (x0 + i * (n / gcd) % n));
    return true;
}
