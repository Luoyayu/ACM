...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < 4; i++) scanf("%d", &keyword[i]);  //存放余数
        for (int i = 0; i < n; i++) scanf("%d", &encoded[i]);  //存放余数

        for (int i = 0; i < n;
             i++)  //求解同余方程组可用CRT的非互质版本，合并上升同余方程
        {
        ...
        while (output[cnt - 1] ==
               ' ')  //处理后面有多个空白的情况。。惨第一次没考虑到
            cnt--;
        for (int i = 0; i < cnt; i++) printf("%c", output[i]);
        printf("\n");
    }
    return 0;
}
