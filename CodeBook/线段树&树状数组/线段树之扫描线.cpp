...
//note:double 离散去重；不能用点来维护线段的长度，因为点此时维护的信息不再是本条线段的长度，而是该点到下个点的长度。注意每次计算长度时X[r+1]-X[l]；
//实现：将矩形的上下边分为上位边，下位边按高度排序得到若干条扫描线
//将横坐标离散化，按扫描线y坐标从小到大读入扫描线 ，维护当前扫描线能有效覆盖的x轴的最大长度sum[i]
//note:设置标记cnt;扫描线碰到下位边cnt++;碰到下位边cnt--;if cnt!=0; 表示当前节点控制的区间仍然被覆盖
//建立node 保存每条扫描线l:扫描线左端点x坐标，r:扫描线右端点x坐标,h:扫描线高度,d:标记
...
int cnt[maxn << 2];    //cnt>=0 表示本节点控制的区域下的下位边个数-上位边的个数，cnt==-1,表示本节点的左右子节点的上下位边数不一致
...
int main()
{
    ...
    while (scanf("%d", &q) != EOF && q)
    {
        ...
        for (int i = 1; i < m; i++)
        {
            ...
            if (l <= r)
                update(l, r, node[i].d, root);
            ret += sum[1] * (node[i + 1].h - node[i].h); //sum[1]所有扫描线能有效覆盖的z轴的最大长度
        }
