...
$$A(x)=\sum_{j=0}^{n-1}a_jx^j ,\ B(x)=\sum_{j=0}^{n-1}b_jx^j$$($n$为多项式项数,记$ degree(A)=k$)

任何一个大于一个多项式次数的整数都是该多项式的***次数界***

​                              $$C(x)=A(x)×B(x)=\sum_{j=0}^{2n-2}(\sum_{k=0}^{j}a_kb_{j-k})x^j$$ 
...
​                       $$c_j=\sum_{j=0}^{2n-2}(\sum_{k=0}^{j}a_kb_{j-k})x^j$$  $$c[j+k]=\sum_{j=0}^{n-1}\sum_{k=0}^{n-1}a_j b_k$$
